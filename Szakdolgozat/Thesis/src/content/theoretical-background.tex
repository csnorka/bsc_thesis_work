...
\section{A ,,To-Be'' Folyamat Logikai Tervezése (BPMN 2.0)}
\label{sec:bpmn_design}

Mielőtt a specifikus szoftverkomponensek (SAP Build) implementációs részleteit tárgyalnám, elengedhetetlen a tervezett automatizált folyamat szoftverfüggetlen, logikai modelljének definiálása. A folyamattervezéshez az ipari szabványnak tekinthető \textbf{BPMN 2.0 (Business Process Model and Notation)} jelölésrendszert alkalmaztam.

A \ref{fig:bpmn_logic}. ábra mutatja be a megtervezett ,,To-Be'' folyamat logikai ívét. Ez a modell absztrahálja a technikai megvalósítást, és tisztán az üzleti logikára, a döntési pontokra és az adatfolyamra koncentrál.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/Swimline.png} 
    \caption{A tervezett automatizált (To-Be) folyamat szoftverfüggetlen BPMN 2.0 modellje.}
    \label{fig:bpmn_logic}
\end{figure}

A modell az alábbi logikai szakaszokra bontja a folyamatot:
\begin{enumerate}
    \item \textbf{Eseményvezérelt indítás (Triggering):} A folyamatot nem manuális indítás, hanem egy külső esemény (számla beérkezése) aktiválja.
    \item \textbf{Automatikus Adatfeldolgozás:} Az első szakaszban (Data Processing) nincs emberi interakció; a rendszer autonóm módon strukturálja az adatokat.
    \item \textbf{Döntési Kapuk (Gateways):} A rombusz alakzatok jelölik a kritikus elágazásokat. A modellben jól látható a kétlépcsős szűrés: először a technikai validáció (\textit{Is Data Valid?}), majd az üzleti szabály (\textit{Amount > Limit?}).
    \item \textbf{Hibrid Végrehajtás:} A folyamat dinamikusan vált a teljesen automatikus és az emberi beavatkozást igénylő (\textit{Manager Approval}) lépések között.
\end{enumerate}

Ez a szabványos modell szolgált alapul a későbbi, 4.3. fejezetben részletezett technikai implementációhoz az SAP Build környezetben.

\subsection{Robotikus Folyamatautomatizálás (RPA) -- A Taktikai Eszköz és Korlátai}
\label{ssec:rpa_theory}

Míg a BPM a folyamatok stratégiai ,,karmestere'', addig a Robotikus Folyamatautomatizálás (Robotic Process Automation, RPA) a ,,digitális munkás'', amely a konkrét, repetitív feladatokat végzi el. Az RPA olyan szoftvertechnológia, amely lehetővé teszi szoftverrobotok (botok) építését, amelyek az emberi felhasználói felületi (UI) interakciókat (kattintás, gépelés, adatbevitel) utánozzák \cite{43, 44}.

Az RPA ideális taktikai megoldást kínál az 1. fejezetben azonosított ,,manuális munka fogságában'' lévő pénzügyi osztályok számára. A ,,forgószék'' (swivel-chair) probléma -- ahol az ügyintéző adatokat másol egy PDF-ből vagy Excelből egy SAP tranzakcióba -- tökéletes célpontja az RPA-nak. A Gartner \cite{2} rámutat, hogy az RPA értéke nem csupán a megtakarított munkaórákban rejlik, hanem a pontosság és a megfelelőség javításában is.

Az RPA botoknak két fő típusa van \cite{41, 5}:
\begin{itemize}
...
\subsubsection*{Mérnöki Kritika: Az RPA Technológiai Korlátai}
...
Éppen ezen korlátok miatt hangsúlyozzák a szakértők \cite{24}, hogy az RPA nem helyettesíti, hanem kiegészíti a BPM-et: az RPA a ,,hogyan''-t oldja meg gyorsan, de törékenyen, míg a BPM és a modern API-alapú integrációk a ,,miért''-et és a stabilitást biztosítják.
